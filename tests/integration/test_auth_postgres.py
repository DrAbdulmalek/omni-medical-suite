...
async def test_concurrent_refresh_rotation_is_single_use_postgres():
    db_session.init_db()
    assert db_session.AsyncSessionLocal is not None, "PostgreSQL session factory was not initialized"
    ...
    try:
        ...
        async with db_session.AsyncSessionLocal() as db:
            ...
            token_result = await db.execute(select(RefreshToken).where(RefreshToken.user_id == user.id))
