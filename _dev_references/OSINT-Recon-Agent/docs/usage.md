...
AI Assistant: Quick scope check: is this a target you own or have written
...
AI Assistant: [proceeds normally]
```
...
AI Assistant: [no scope check needed; proceeds with §7 pipeline]
```
...
AI Assistant: [pulls §7.6 1-week deep profile + §10.1 medium-org tactics +
...
AI Assistant: [pulls §7.5 priority order + §8.5 asset-level triage:
...
AI Assistant: [pulls §17 catalog (CRITICAL severity), §6.3 validator discipline,
...
AI Assistant: [pulls §31.1 exec summary template + §31.3 risk translation matrix,
...
Add "show which sections you're using" to your prompt. AI Assistant will cite §s, which helps you trust the answer and learn the skill structure:

```
You: How do I find an origin behind Cloudflare? Show which sections you're using.
AI Assistant: [pulls methodology §27 + arsenal §16.15, cites both, walks through 8 techniques]
```
...
AI Assistant will re-evaluate against §7.5 priority + §8.5 triage rules.

### Confidence-grade your findings

```
You: I think this subdomain is a takeover candidate. How confident should I be?
AI Assistant: [pulls §2 confidence levels + §16.12 takeover signatures, evaluates]
```
...
AI Assistant: [pulls §6.4 detection-aware probing, walks through back-off ladder]
```
...
- ❌ Asking AI Assistant to *execute* probes. AI Assistant doesn't have access to your network. It tells you what to run; you run it.
- ❌ Pasting real PII / credentials / breach corpus content into the prompt. Use placeholder data.
- ❌ Skipping the scope check. If the engagement isn't authorized, AI Assistant shouldn't (and won't) help with active probing.
- ❌ Treating AI Assistant's output as ground truth without verification. Always validate against the §17 catalog (regex match), §40 severity matrix (worked examples), and your own engagement context.
