...
| PR | فتح Pull Request إلى main (قرار Release Manager = C) | DONE | PR #136 | n/a | https://github.com/DrAbdulmalek/omni-medical-suite/pull/136 — head=fd765ff، labels×7، بوابات الدمج G1-G6 مضمَّنة في الوصف. حماية main فعالة: review×1 + 8 required checks (strict) + enforce_admins |
| ATR-04d | حراس CI لاختبارات ATR (importorskip) | DONE | (this commit) | 38/38 محليًا + 17p/4s بمحاكاة CI | سبب فشل `Unit Tests` في CI (مثبت من سجل الوظيفة): collection errors — `No module named 'torch'/'flask'` (بيئة CI تثبت api+ml فقط). الحل: `pytest.importorskip` في 3 ملفات اختبار (+yaml في docker) — **بلا إضعاف**: مع التبعيات 38/38 كامل، وبدوها skips نظيفة. لا تعديل على منصة CI نفسها |
| ATR-07c | Report-Integrity Corrections (مراجعة تدقيقية خارجية) | DONE | bd0bf75 (+stamp child) | n/a | 9 تصحيحات توثيقية: VERIFICATION block بـ SHAs كاملة + MATCH؛ فصل declared-vs-validated لـ transformers؛ صياغة Git دقيقة للـadditive؛ تقسيم deps (جديد vs معاد استخدامه)؛ إزالة بصمة التوكن؛ tiered persistence (Git PROVEN / bundles local / remote archival NOT ESTABLISHED)؛ ATR-03 real-load "Z.ai-env only"؛ لا تغيير كود/اختبارات بعد ba77ba4 |
...
- بيانات اعتماد GitHub: ملف محلي موجود (بصمته محفوظة في سجل الجلسة، **لا تُعاد هنا** لأسباب أمنية) — يُستخدم عبر credential helper مؤقت، **لا يُطبع أبدًا**.
