...
تشغيل 3 محركات OCR مع تجمع ذكي، تصحيح الكلمات،
وحفظ بيانات التدريب تلقائياً.

المحسّن لـ Hugging Face Spaces Free Tier:
- مسارات /data/ للتخزين الدائم
- 3 محركات فقط (PaddleOCR + EasyOCR + Tesseract)
- بدون TrOCR/Surya (يحتاجان RAM أعلى)
...
# ============================================================
# على Hugging Face Spaces، /data/ هو المجلد الوحيد الدائم (persisted)
# المجلدات النسبية تُفقد عند إعادة تشغيل الحاوية
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Ensemble — 3 Engines (HF Free Tier)")
        ...
        # المحركات المتاحة على المستوى المجاني (بدون TrOCR/Surya)
        all_engines = ["paddleocr", "easyocr", "tesseract"]
    ...
    st.caption("3 Engines + Smart Merging | PaddleOCR + EasyOCR + Tesseract | HF Free Tier Optimized")
    ...
    with tab1:
        ...
        if not uploaded:
            st.markdown(
                ...
                ### 📤 ارفع مستندك الطبي
                يدعم التطبيق:
                - **3 محركات OCR** مع تجمع ذكي (محسّن للـ HF Free Tier)
