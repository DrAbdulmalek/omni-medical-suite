...
from packages.security.user_rate_limiter import UserRateLimiter, RateLimitExceeded
...
class TestEndToEndOCRWorkflow:
    ...
    async def test_ocr_with_circuit_breaker_fallback(self, mock_redis):
        ...
        async def mistral_call():
            return await mistral_cb.call(lambda: asyncio.sleep(0))
...
class TestPerformanceWorkflow:
    ...
    async def test_circuit_breaker_fast_fail(self, mock_redis):
        ...
        start = time.time()
        try:
            await cb.call(lambda: asyncio.sleep(10))  # Would take 10s
