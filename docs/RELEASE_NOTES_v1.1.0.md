...
1. **AppImage numpy/OpenBLAS crash (CRITICAL)**: The v1.1.0 AppImage was built with numpy 2.4.6, which bundles `libscipy_openblas64_` that fails to load in PyInstaller onefile mode on many Linux systems. The error is: `ImportError: libscipy_openblas64_-32a4b2a6.so: ELF load command address/offset not page-aligned`. **Root cause**: `packages/scanner_fixer/pyproject.toml` declared `numpy>=1.23` with no upper bound; pip's resolver upgraded numpy to 2.x during `pip install -e packages/scanner_fixer`, overriding the desktop pin. **Fix**: Pinned `numpy>=1.24.0,<2.0.0` in `packages/scanner_fixer/pyproject.toml`, `hf-space/packages/scanner_fixer/pyproject.toml`, `requirements-scanner.txt`, and root `pyproject.toml`. The runtime hook (`hook_numpy_openblas.py`) is retained as defense-in-depth. See `docs/KNOWN_ISSUES.md` for the full write-up. Users on v1.1.0/v1.1.1 can run `bash scripts/rebuild-appimage.sh` locally as a workaround; the fix ships in v1.2.0.
