...
def compute_accuracy(prediction: str, reference: str) -> float:
    ...
    if len(prediction) != len(reference):
        return 0.0
    correct = sum(p == r for p, r in zip(prediction, reference, strict=False))
...
def compute_batch_metrics(predictions: list[str], references: list[str]) -> dict:
    ...
    total_cer = sum(compute_cer(p, r) for p, r in zip(predictions, references, strict=False))
    total_wer = sum(compute_wer(p, r) for p, r in zip(predictions, references, strict=False))
