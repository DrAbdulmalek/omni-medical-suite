...
### Fixed
- `hf_app.py` — نقل `theme` و `css` من `demo.launch()` إلى `gr.Blocks()` (Gradio 6 compatibility)
- `requirements-hf.txt` — إضافة `pytesseract` و `python-docx` و `openpyxl` المفقودة
- `Dockerfile` — تصحيح healthcheck endpoint و CMD لـ Gradio بدلاً من Streamlit
- `pyproject.toml` — تحديث الإصدار من 4.2.0 إلى 4.3.0

### Changed
- `README.md` — تحديث رقم الإصدار إلى v4.3.0
