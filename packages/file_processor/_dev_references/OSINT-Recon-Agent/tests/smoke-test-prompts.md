...
32 verification prompts to confirm the skills load and behave correctly after install. Drop each into a fresh AI Assistant session and verify the **expected behavior**.
...
3. Paste each prompt.
4. Check AI Assistant's response against "Expected behavior".
...
| 5 | "Generate cloud bucket candidates for 'Shree Cement Limited' with subdomains api/billing/hr/intranet." | Pulls arsenal §16.8; produces seed derivation + applies 6 prefixes × 15 suffixes. (Acceptable: AI Assistant does runtime synthesis; may not produce literal 720-line list.) |
...
| 32 | "Found `sk-ant-api03-...` in a JS bundle. What is it + how serious?" | Pulls arsenal §17 row 30 (AI Provider API key, CRITICAL) + §23.5 (read-only validator) + §23.12 (post-validation enum). |
...
- **Hallucinated endpoint / regex / wordlist** — AI Assistant invented something. Flag the prompt; tighten the section it should have pulled from with explicit "do not invent" language.
...
- **Severity inflation** — AI Assistant calling everything CRITICAL. Re-anchor on §40 worked examples.
- **Severity deflation** — AI Assistant calling `.env` exposure MEDIUM. Same fix.
