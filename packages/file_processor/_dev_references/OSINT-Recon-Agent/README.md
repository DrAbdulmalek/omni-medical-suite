![ai-osint banner](assets/banner.png)

# ai-osint

> 2 paired AI Assistant skills · **90+ recon modules** · 48 secret-regex patterns · 80+ dorks · 9 read-only credential validators · 27 attack-path templates · 5,500+ lines of structured tradecraft. Drop-in `SKILL.md` files that turn AI Assistant into a god-mode external recon operator for authorized red-team and bug-bounty engagements.
...
`ai-osint` is a paired set of skills for the [AI Assistant skills system](https://docs.example-ai.com/en/docs/ai-assistant-code/skills). Each skill is a structured `SKILL.md` file that primes AI Assistant with expert-level methodology for one half of the offensive recon problem:
...
Drop both into your AI Assistant environment and it behaves like a senior recon analyst: it knows the techniques, the tooling, the edge cases, and the escalation paths — and it stays in scope.
...
Each skill directory is self-contained. Drop into `~/.ai-skills/` and AI Assistant auto-triggers on relevant phrases.

---

## Skill Index

90+ capabilities across 12 domains. Categorized like AI Assistant-Red — pick a domain to drill in.
...
| Modern AI API keys (AI Provider / OpenAI / HuggingFace / Cloudflare) | arsenal (rows 30-36) |
| Package-registry tokens (npm / PyPI / Docker Hub) | arsenal (rows 38-40) |
| GitHub code-search dorks (13 templates) | arsenal |
| 9 read-only credential validators (Postman / AWS / GitHub / Slack / AI Provider / OpenAI / npm / Atlassian / DataDog) | arsenal |
...
cp -r AI-OSINT/skills/* ~/.ai-skills/
```

Then in any AI Code session, ask an OSINT question — both skills auto-load and trigger on relevant phrases (50+ trigger phrases each).

### With the AI Assistant Skills System

```bash
# Point AI Assistant at a single skill before starting your session
cat skills/offensive-osint/SKILL.md | ai-assistant --system-file -
```

### Manual (AI Platform / AI API)

Paste the contents of any `SKILL.md` into a Project's system prompt or prepend it to your conversation. Both files are plain Markdown — also usable as a personal cheat-sheet without AI Assistant.
...
Both skills include a soft scope-check when you ask AI Assistant to act against an unverified third-party target. They explicitly **exclude** active exploitation, post-exploitation, malware development, and other activities beyond OSINT-driven reconnaissance. See [`SECURITY.md`](SECURITY.md) for the full posture.
...
Operational tradecraft accumulated across external attack-surface engagements, codified into AI Assistant skills. Engagement-platform agnostic - slot into any ASM / ticketing / asset-graph platform you already use, or none.
...
> *"Give AI Assistant the right skill and it stops being a chatbot. It becomes an operator."*
