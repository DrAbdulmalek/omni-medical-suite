...
.PHONY: help dev build test test-py test-web lint lint-py lint-web \
        ...
        lite standard prod mode-down mode-status
...
## lite - Start Lite mode (web + api only, SQLite)
lite:
	@echo "$(CYAN)Starting OmniMedical Suite in LITE mode...$(RESET)"
	cp -n .env.lite .env 2>/dev/null || true
	docker compose -f docker-compose.lite.yml up --build -d
	@echo "$(GREEN)Lite mode running: http://localhost:3000$(RESET)"

## standard - Start Standard mode (full stack, PostgreSQL)
standard:
	@echo "$(CYAN)Starting OmniMedical Suite in STANDARD mode...$(RESET)"
	cp -n .env.standard .env 2>/dev/null || true
	docker compose -f docker-compose.standard.yml up --build -d
	@echo "$(GREEN)Standard mode running: http://localhost:3000$(RESET)"

## prod - Start Production mode (all services, resource limits)
prod:
	@echo "$(CYAN)Starting OmniMedical Suite in PRODUCTION mode...$(RESET)"
	@echo "$(YELLOW)Make sure .env has production-safe secrets!$(RESET)"
	docker compose -f docker-compose.prod.yml up --build -d
	@echo "$(GREEN)Production mode running$(RESET)"

## mode-down - Stop current deployment mode
mode-down:
	docker compose -f docker-compose.lite.yml down --remove-orphans 2>/dev/null || true
	docker compose -f docker-compose.standard.yml down --remove-orphans 2>/dev/null || true
	docker compose -f docker-compose.prod.yml down --remove-orphans 2>/dev/null || true

## mode-status - Show running services across all modes
mode-status:
	@echo "=== Lite Mode ==="
	docker compose -f docker-compose.lite.yml ps 2>/dev/null || echo "Not running"
	@echo ""
	@echo "=== Standard Mode ==="
	docker compose -f docker-compose.standard.yml ps 2>/dev/null || echo "Not running"
	@echo ""
