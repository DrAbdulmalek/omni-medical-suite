...
def _env_files() -> tuple[Path, ...]:
    ...
    files: list[Path] = [
        Path.home() / ".config" / "omnifile-ai-gateway" / ".env",
        Path(".env"),
    ]
    if explicit := os.environ.get("OAG_ENV_FILE"):
...
class Settings(BaseSettings):
    ...
    assistant_workspace: str = "./agent_workspace"
    allowed_dir: str = ""
    assistant_cli_bin: str = Field(default="claude", validation_alias="ASSISTANT_CLI_BIN")
    ...
    def resolve_model(self, assistant_model_name: str) -> str:
        ...
        """
        name_lower = assistant_model_name.lower()
    ...
    def resolve_thinking(self, assistant_model_name: str) -> bool:
