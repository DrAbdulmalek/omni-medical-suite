...
# -*- coding: utf-8 -*-
"""
معالج الوثائق الطبية التفاعلي - v15 (نسخة موحدة)
═══════════════════════════════════════════════════════════════
جديد v15 (دمج v13 + v13.1_patch + v14_fixed):
  1. find_page_bounds — كشف الحدود من جميع الجهات (4 اتجاهات)
     باستخدام mean/median هجين للتعامل مع النص الكثيف
  2. auto_detect_skew — كشف دقيق بدون قيم قصوية خاطئة
     نطاق ±5° بدلاً من ±15°، CLAHE، تحقق من 0°
  3. smart_auto_crop — يستخدم find_page_bounds المحسّن (4 اتجاهات)
  4. _apply_auto_deskew_on_load — محسّن مع التحقق من الميلان الحقيقي
  5. _on_auto_skew_done — إعادة تعيين الميلان إذا كان ضئيلاً

موروث من v13:
  - RegionSelectorLabel / RegionSelectorDialog
  - extract_page_number مع دعم مناطق مخصصة
  - QTimer batch (non-blocking), _cancel_batch
  - حفظ ذكي (OCR + ترقيم), page_registry
  - LazyImage, QMutex + _is_processing, gray_threshold slider
...
def find_page_bounds(img: np.ndarray,
    ...
    التحسينات في v15 (من v13.1 patch):
    ...
    # ── تحليل الصفوف (علوي/سفلي) — جديد في v15! ──────────
...
class MedicalDocApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🏥 معالج الوثائق الطبية v15 — نسخة موحدة")
    ...
    def _apply_auto_deskew_on_load(self):
        """Auto deskew with improved detection and validation (v15)."""
    ...
    def _on_auto_skew_done(self, angle: float):
        """Handle auto deskew completion with validation (v15)."""
