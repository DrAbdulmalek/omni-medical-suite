...
# -*- coding: utf-8 -*-
"""
معالج الوثائق الطبية التفاعلي - v13 (النسخة النهائية)
═══════════════════════════════════════════════════════════════
جديد v13 (مدمج من جميع الإصدارات + region_selector.py):
  1. RegionSelectorLabel  — تحديد منطقة بالماوس مباشرة على الصورة
  2. RegionSelectorDialog — حوار تفاعلي لتحديد مناطق متعددة
  3. extract_page_number  — يدعم مناطق مخصصة (region/regions)
  4. _select_page_number_region — تحديد منطقة رقم الصفحة مع تصحيح offset
  5. _test_page_number_region   — اختبار OCR على جميع المناطق
  6. دعم مناطق متعددة لكتب أرقامها في أماكن مختلفة

موروث من v12:
  - QTimer batch (non-blocking), _cancel_batch, _set_controls_enabled
  - _on_auto_save_toggle, _save_screenshot
  - حفظ ذكي (OCR + ترقيم), page_registry

موروث من v10.1:
  - LazyImage, find_page_bounds, auto_detect_skew (إزالة الرمادي أولاً)
  - smart_auto_crop ثنائي المراحل, _safe_move
  - QMutex + _is_processing, gray_threshold slider
  - logger موحّد, TrainingDataCollector, AdaptiveLearner (KNN)
═══════════════════════════════════════════════════════════════
"""
import sys
import csv
...
def find_page_bounds(img: np.ndarray,
    ...
    المرحلة 1 — الأعمدة (يمين/يسار):
        الرمادي (الماسح): median الأعمدة ≈ 150–160  (<  page_threshold)
        الصفحة البيضاء: median الأعمدة ≈ 254–255  (>  page_threshold)

    المرحلة 2 — الصفوف (أعلى/أسفل):
        نحلل الصفوف فقط في حواف الصفحة المكتشفة (أول 15% وآخر 15%)
        لأن النص موجود في المنتصف ولن يُخطئ.
        نستخدم mean بدل median لأن الرمادي منتظم.

    خوارزمية "أكبر كتلة متصلة" تتجنب المناطق البيضاء المزيفة.

    يُرجع (l, t, r, b) — قص من جميع الجهات.
    ═══════════════════════════════════════════════════════════════
    """
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if img.ndim == 3 else img
    h, w  = gray.shape[:2]
    ...
    # ── المرحلة 2: الصفوف (أعلى/أسفل) ─────────────────────────
    # نحلل فقط في حواف الصفحة المكتشفة (حيث الرمادي موجود)
    # ونستخدم mean بدل median لأن الرمادي منتظم تماماً
    x0, x1 = left, w - (w - right - 1) if (w - right - 1) > 0 else w
    if x1 <= x0:
        return (left, 0, w - right - 1, 0)

    row_means = np.mean(gray, axis=1)          # (h,) mean لكل صف
    top = 0
    bottom = h - 1

    # إزالة الحدود الرمادية من الأعلى
    # نبحث في أول 30% من الصفوف عن صفوف رمادية (mean < page_threshold)
    top_limit = max(int(h * 0.30), 50)
    for y in range(min(top_limit, h)):
        if row_means[y] > page_threshold:
            top = max(0, y - MARGIN)
            break

    # إزالة الحدود الرمادية من الأسفل
    bottom_limit = max(int(h * 0.30), 50)
    for y in range(min(bottom_limit, h - 1), -1, -1):
        if row_means[y] > page_threshold:
            bottom = min(h - 1, y + MARGIN)
            break

    # تأكد أن top و bottom منطقيان
    if top >= bottom:
        top, bottom = 0, h - 1
...
def auto_detect_skew(img: np.ndarray, max_a: float = 15.0, step: float = 0.5) -> float:
    ...
    ✅ يستخدم BORDER_CONSTANT (أبيض) بدل BORDER_REPLICATE → لا تباين كاذب.
    ✅ يتطلب تحسناً فعلياً > 5% عن 0° قبل قبول زاوية غير صفرية.
    ✅ يُضيف عُقوبة للزوايا المتطرفة (> 5°) لمنع الإيجابيات الكاذبة.

    المشكلة السابقة:
      BORDER_REPLICATE يُنتج تباين عالي كاذب عند الدوران بزوايا كبيرة
      مما يجعل ±15° (حدود البحث) تبدو كأفضل زاوية دائماً.

    الحل:
      1. نحسب النتيجة عند 0° كمرجع (baseline)
      2. نستخدم BORDER_CONSTANT مع خلفية بيضاء
      3. نطلب تحسناً حقيقياً > 5% فوق المرجع
      4. نُضيف عُقوبة تتناسب طردياً مع الزاوية (linear penalty)
    """
    ...
    page = img[y0:y1, x0:x1] if (x1 > x0 and y1 > y0) else img
    ...
    # CENTER: نستخدم منتصف الصورة فقط (50% عرض، 80% ارتفاع)
    # لتجنب تأثير حواف النص والحشو
    cx0 = pw // 4
    cx1 = 3 * pw // 4
    cy0 = ph // 10
    cy1 = 9 * ph // 10
    ...
    for angle in np.arange(-max_a, max_a + step, step):
        M = cv2.getRotationMatrix2D((cw // 2, ch // 2), angle, 1.0)
        rot = cv2.warpAffine(center, M, (cw, ch),
                             flags=cv2.INTER_LINEAR,
                             borderMode=cv2.BORDER_CONSTANT,
                             borderValue=(0,))          # خلفية سوداء (مثل النص)

        # استخدام الانحراف المعياري بدل التباين (أكثر استقراراً)
        row_sums = np.sum(rot, axis=1).astype(np.float64)
        score = float(np.std(row_sums))

        # عُقوبة خطية للزوايا المتطرفة
        # كل درجة تُزيل 1% من النتيجة → زاوية 10° تخسر 10%
        penalty = 1.0 - (abs(angle) / 100.0)
    ...
    # ── المرحلة 3: التحقق من الثقة ─────────────────────────────
    # إذا لم نحصل على نتيجة 0° صريحة، نحسبها الآن
    if zero_score < 0:
        M0 = cv2.getRotationMatrix2D((cw // 2, ch // 2), 0, 1.0)
        rot0 = cv2.warpAffine(center, M0, (cw, ch),
                              flags=cv2.INTER_LINEAR,
                              borderMode=cv2.BORDER_CONSTANT,
                              borderValue=(0,))
        row_sums_0 = np.sum(rot0, axis=1).astype(np.float64)
        zero_score = float(np.std(row_sums_0))

    # إذا كانت الزاوية المتوقعة ضمن ±0.5° → صفر مباشرة
    if abs(best_angle) <= 0.5:
        return 0.0

    # حساب النتيجة عند الزاوية المقترحة بدون عُقوبة للمقارنة الحقيقية
    M_check = cv2.getRotationMatrix2D((cw // 2, ch // 2), best_angle, 1.0)
    rot_check = cv2.warpAffine(center, M_check, (cw, ch),
                                 flags=cv2.INTER_LINEAR,
                                 borderMode=cv2.BORDER_CONSTANT,
                                 borderValue=(0,))
    actual_score = float(np.std(np.sum(rot_check, axis=1).astype(np.float64)))

    # التحقق: يجب أن يكون التحسن > 5% فوق نتيجة 0°
 min_improvement = 1.05  # 5% تحسن مطلوب
    if zero_score > 0 and actual_score < zero_score * min_improvement:
        logger.debug("ميلان مرفوض: %.1f° (تحسن %.1f%% < 5%%)", best_angle,
                       (actual_score / zero_score - 1) * 100)
        return 0.0

    # لا تقبل زوايا > 5° إلا مع تحسن قوي (> 15%)
    if abs(best_angle) > 5.0 and zero_score > 0:
        strong_improvement = 1.15  # 15% تحسن مطلوب للزوايا الكبيرة
        if actual_score < zero_score * strong_improvement:
            logger.debug("ميلان متطرف مرفوض: %.1f° (تحسن %.1f%% < 15%%)", best_angle,
                           (actual_score / zero_score - 1) * 100)
            return 0.0
...
def smart_auto_crop(img: np.ndarray, padding: int = 15, dark_threshold: int = 200) -> tuple:
    ...
    # المرحلة 1: إزالة الرمادي (من جميع الجهات الآن)
    gl, gt, gr, gb = find_page_bounds(img)
    x0 = gl
    x1 = w - gr if gr > 0 else w
    y0 = gt
    y1 = h - gb if gb > 0 else h
    if x1 <= x0 or y1 <= y0:
        return (gl, gt, gr, gb)
    page = img[y0:y1, x0:x1]
    ph, pw = page.shape[:2]

    # المرحلة 2: كشف المحتوى داخل الصفحة النظيفة
    gray = cv2.cvtColor(page, cv2.COLOR_BGR2GRAY) if page.ndim == 3 else page
    _, binary = cv2.threshold(gray, dark_threshold, 255, cv2.THRESH_BINARY_INV)

    col_has = binary.max(axis=0) > 0       # (pw,) bool
    row_has = binary.max(axis=1) > 0       # (ph,) bool
...
def extract_page_number(img: np.ndarray,
                         region=None, regions=None) -> int:
    """
    Extract page number using OCR.

    Args:
        region:  (x_frac, y_frac, w_frac, h_frac) — نسب من أبعاد الصورة
        regions: قائمة من المناطق للكتب ذات أرقام في أماكن متعددة
        إذا لم يُعطَ شيء: يبحث في الزوايا الأربع والأشرطة الأفقية
    """
    if not OCR_SUPPORT:
        return 0
    ...
    # بناء قائمة المناطق للبحث
    search_regions = []
    if regions:
        for r in regions:
            x = int(r[0] * w); y = int(r[1] * h)
            rw = int(r[2] * w); rh = int(r[3] * h)
            if rw > 0 and rh > 0 and x < w and y < h:
                search_regions.append(gray[y:y+rh, x:x+rw])
    elif region:
        x = int(region[0] * w); y = int(region[1] * h)
        rw = int(region[2] * w); rh = int(region[3] * h)
        if rw > 0 and rh > 0 and x < w and y < h:
            search_regions.append(gray[y:y+rh, x:x+rw])
    else:
        # المواضع الافتراضية — تغطي الأنماط الشائعة للكتب العربية
        bot  = max(100, h // 10)
        top  = max(100, h // 10)
        search_regions = [
            gray[h-bot:h, :],          gray[h-bot:h, w//3:2*w//3],
            gray[h-150:h, w-200:w],    gray[h-150:h, 0:200],
            gray[0:top, :],            gray[0:top, w//3:2*w//3],
            gray[0:150, w-200:w],      gray[0:150, 0:200],
        ]
    ...
    for rgn in search_regions:
        ...
        try:
            ...
            _, enhanced = cv2.threshold(enhanced, 0, 255,
...
# ════════════════════════════════════════════════════════════════
#  منتقي المنطقة — لتحديد موضع رقم الصفحة يدوياً
# ════════════════════════════════════════════════════════════════

class RegionSelectorLabel(QLabel):
    """
    QLabel يدعم رسم مربع تحديد بالماوس مباشرة على الصورة.
    يُرسل إشارة region_selected بإحداثيات المربع.
    """
    ...
    def paintEvent(self, event):
        ...
        if self._current_rect and not self._active:
...
class RegionSelectorDialog(QDialog):
    """
    حوار تفاعلي لتحديد منطقة رقم الصفحة.
    يدعم مناطق متعددة للكتب ذات أرقام في أماكن مختلفة.
    """
    def __init__(self, pixmap: QPixmap, parent=None):
        ...
        self.lbl_hint = QLabel("💡 اضغط 'ابدأ التحديد' ثم ارسم مربعاً حول رقم الصفحة")
        self.lbl_hint.setStyleSheet("color:#475569;padding:4px;")
    ...
    def _test_ocr(self):
        ...
        QMessageBox.information(
            self, "منطقة محددة",
            "المنطقة: x={}, y={}, عرض={}, ارتفاع={}\n\n"
            "اضغط 'حفظ المنطقة' للتأكيد.".format(r.x(), r.y(), r.width(), r.height()))
...
class MedicalDocApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🏥 معالج الوثائق الطبية v13 — النسخة النهائية")
        ...
        self._page_number_regions: list = []   # ← مناطق رقم الصفحة المحددة يدوياً   # رقم الصفحة → بيانات الملف
    ...
    def _build_ui(self):
        ...
        self.btn_select_region  = self._mk_btn("📍 منطقة رقم الصفحة", "#0891b2", w=140)
        ...
        for w in [self.lbl_status, None, self.lbl_index,
                  ...
                  self.btn_analyze_pages, self.btn_select_region, self.btn_open]:
            if w is None:
                top.addStretch()
            else:
                top.addWidget(w)
        ...
        self.btn_cancel_batch.setVisible(False)  # مخفي حتى تبدأ الدُفعة
    ...
    def _load_current(self):
        """Load the current image and prepare for processing."""
        if not self.image_list:
            return
        entry = self.image_list[self.current_idx]
        name = self.image_names[self.current_idx]
        ...
        if isinstance(entry, Path) and not entry.exists():
            self._log("⚠️ الملف {} غير موجود. سيتم حذفه.".format(entry.name))
            self.image_list.pop(self.current_idx)
            self.image_names.pop(self.current_idx)
            if self.current_idx < len(self.image_paths):
                self.image_paths.pop(self.current_idx)
        ...
        # Reset rotation and operation history for each new image
        self.current_params["rotation"] = 0
        self.operation_history = []
        ...
        if self.chk_learn.isChecked():
            ...
            if t_params:
                self.current_params.update(t_params)
                self._log("🧠 تنبؤ من بيانات التدريب ({}%): {}".format(int(t_sim * 100), name))
            else:
                a_params, a_sim = self.learner.suggest(img)
                if a_params:
                    self.current_params.update(a_params)
                    self._log("🤖 اقتراح التعلّم ({}%): {}".format(int(a_sim * 100), name))
                else:
                    self._log("📄 تحميل: {}".format(name))
        else:
            self._log("📄 تحميل: {}".format(name))
        ...
        # Auto-deskew on load if enabled
        if hasattr(self, 'chk_auto_deskew') and (self.chk_auto_deskew.isChecked() or self.auto_save_enabled):
            self._apply_auto_deskew_on_load()
        else:
            self._update_preview()
    ...
    def _sync_params_to_ui(self):
        """Synchronize UI controls from current_params (alias)."""
        self._sync_ui_from_params()

    def _sync_ui_to_params(self):
        """Collect current UI values into current_params (alias)."""
    ...
    def _auto_save_step(self):
        ...
        try:
            ...
            if img is None:
                ...
                # ── حفظ ذكي: OCR مع مناطق المستخدم المحددة ────────
                save_name = name
                if self.chk_smart_save.isChecked() and OCR_SUPPORT:
                    page_num = extract_page_number(
                        processed,
                        regions=self._page_number_regions if self._page_number_regions else None)
                    if page_num > 0:
                        save_name = "page_{:04d}.png".format(page_num)
                        self.page_registry[page_num] = {
                            'quality': blur_a, 'path': str(crop_base / save_name),
                            'name': name,
                        }
    ...
    def _set_controls_enabled(self, enabled: bool):
        ...
        targets = [
            ...
            self.btn_fullscreen, self.btn_remove_gray, self.btn_select_region,
        ]
        for btn in targets:
    ...
    def _select_page_number_region(self):
        """
        يفتح حوار تحديد منطقة رقم الصفحة بالماوس.
        يدعم إضافة مناطق متعددة لكتب أرقام صفحاتها في أماكن متغيرة.
        يُصحح offset الإحداثيات بين الـ Label والـ Pixmap (AlignCenter).
        """
        if self.current_img is None:
            QMessageBox.warning(self, "تنبيه", "لا توجد صورة محملة.")
            return

        pix = cv2_to_pixmap(self.current_img, max_w=900, max_h=680)
        ...
        # ── تصحيح offset: الـ Label أكبر من الـ Pixmap (AlignCenter) ──────
        pix_w  = pix.width()
        pix_h  = pix.height()
        lbl_w  = dlg.label.width()
        lbl_h  = dlg.label.height()
        offset_x = max(0.0, (lbl_w - pix_w) / 2.0)
        offset_y = max(0.0, (lbl_h - pix_h) / 2.0)
        ...
        # قص على حدود الصورة
        rx = max(0.0, min(rx, pix_w))
        ry = max(0.0, min(ry, pix_h))
        rw = max(0.0, min(rw, pix_w - rx))
        rh = max(0.0, min(rh, pix_h - ry))

        if rw < 1 or rh < 1:
            QMessageBox.warning(self, "خطأ", "المنطقة المحددة خارج نطاق الصورة.")
            return

        # تحويل إلى نسب (0-1) → تعمل مع أي حجم صورة
        new_region = (rx / pix_w, ry / pix_h, rw / pix_w, rh / pix_h)
        ...
        for existing in self._page_number_regions:
            if (abs(existing[0] - new_region[0]) < 0.05 and
                ...
                QMessageBox.information(self, "معلومات",
                    "منطقة مشابهة موجودة مسبقاً. تم التجاهل.")
                return

        self._page_number_regions.append(new_region)
        n = len(self._page_number_regions)
        self._log("📍 تم إضافة منطقة رقم الصفحة #{} "
                  "(x={:.2f}, y={:.2f}, w={:.2f}, h={:.2f})".format(
                  n, *new_region))

        reply = QMessageBox.question(
            self, "اختبار OCR",
            "تم حفظ المنطقة #{}/{}.\n\nهل تريد اختبار OCR عليها الآن؟".format(
                n, n),
    ...
    def _test_page_number_region(self):
        """اختبار OCR على جميع المناطق المحددة وعرض النتائج."""
        if not self._page_number_regions:
            QMessageBox.information(self, "معلومات",
                "لا توجد مناطق محددة بعد.\nاضغط '📍 منطقة رقم الصفحة' أولاً.")
            return
        if self.current_img is None:
            return
        ...
        for idx, num in results:
            if num > 0:
                lines.append("  منطقة {}: صفحة {} ✅".format(idx, num))
            else:
                lines.append("  منطقة {}: لم يُعثر على رقم ❌".format(idx))

        summary = "نتائج OCR على {} منطقة:\n\n{}".format(
            len(self._page_number_regions), "\n".join(lines))

        if found_ok > 0:
            summary += "\n\n✅ {} منطقة تعمل بنجاح.".format(found_ok)
        else:
            summary += ("\n\n⚠️ لم يُعثر على أي رقم صفحة.\n"
                        "تأكد من دقة المنطقة المحددة\n"
                        "أو أن Tesseract مثبت ويدعم اللغة العربية.")

        if found_ok > 0:
            QMessageBox.information(self, "نتيجة OCR", summary)
        else:
            QMessageBox.warning(self, "نتيجة OCR", summary)

        self._log("🧪 OCR: {} منطقة، {} نجحت".format(
            len(self._page_number_regions), found_ok))
        ...
        for i in range(self.current_idx, len(self.image_list)):
            try:
                ...
                if isinstance(entry, Path):
                    raw_dest = self._get_unique_path(raw_base, name, ext=entry.suffix)
                    try:
                        shutil.move(str(entry), str(raw_dest))
                    except Exception:
                        pass
    ...
    def analyze_and_organize_pages(self):
        ...
        for idx, entry in enumerate(self.image_list):
            try:
                ...
                elif isinstance(entry, LazyImage):
                    img = entry.get()
                else:
                    img = entry.copy() if hasattr(entry, 'copy') else np.array(entry)

                if img is None:
                    self._log("⚠️ تخطي (قراءة فاشلة): {}".format(self.image_names[idx]))
                    continue

                # Extract page number using user-defined regions if available
                page_num = extract_page_number(
                    img,
                    regions=self._page_number_regions if self._page_number_regions else None)
