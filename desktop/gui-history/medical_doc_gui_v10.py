...
# ==================================================================
# 1. خوارزميات معالجة الصور المتقدمة (بما فيها OCR pipeline)
# ==================================================================
...
# ---------------- خوارزميات OCR المتقدمة (للمستندات الممسوحة) ----------------
def auto_deskew_hough(img, min_line_length=300, max_line_gap=30):
    """
    كشف زاوية الميلان باستخدام خطوط Hough الأفقية (للمستندات).
    ...
    for line in lines:
        ...
        # نأخذ الخطوط القريبة من الأفقي (بين -45 و 45 درجة)
        if -45 <= angle <= 45:
            angles.append(angle)
    if not angles:
        return 0.0
    median_angle = np.median(angles)
    return -median_angle  # الاتجاه المعاكس للتدوير


def smart_ocr_get_crop(img, padding=40, deskew=True):
    """
    خوارزمية متكاملة للمستندات الممسوحة ضوئياً:
      1. تصحيح الميلان (اختياري) باستخدام HoughLinesP.
      2. قص ذكي يقوم بإزالة الهوامش الداكنة للماسح الضوئي.
    تعيد (هوامش القص (l,t,r,b), زاوية الميلان الأصلية).
    """
    original_img = img.copy()
    angle = 0.0

    # 1. تصحيح الميلان إذا طُلب
    if deskew:
        angle = auto_deskew_hough(original_img)
        if abs(angle) > 0.2:
            h, w = original_img.shape[:2]
            center = (w//2, h//2)
            M = cv2.getRotationMatrix2D(center, angle, 1.0)
            # استخدم BORDER_REPLICATE لتجنب هوامش سوداء خارجية
            original_img = cv2.warpAffine(original_img, M, (w, h),
                                          flags=cv2.INTER_CUBIC,
                                          borderMode=cv2.BORDER_REPLICATE)

    # 2. تحويل إلى رمادي وتحضير القناع
    gray = cv2.cvtColor(original_img, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # 3. تمدد كبير لربط الكلمات والسطور والجداول في كتلة واحدة
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 20))
    dilated = cv2.dilate(thresh, kernel, iterations=2)

    # 4. إيجاد الكونتورات الخارجية
    ...
    # 5. إضافة هامش الأمان مع مراعاة حدود الصورة
...
def smart_auto_crop(img, padding=15, dark_threshold=30, use_advanced=True):
    """
    دالة القص الذكي الهجينة (القديمة) – تُستخدم للتوافق.
    في V10 نفضل استخدام smart_ocr_get_crop.
    """
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
    h, w = gray.shape
    ...
    # 2. الانتقال الذكي للخوارزمية المتقدمة
...
def auto_detect_skew_fast(img, max_angle=10, step=0.5):
    """كشف سريع ودقيق للميلان باستخدام تحليل ملفوف الإسقاط (احتياطي)"""
    if img is None:
        return 0.0
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
    h, w = gray.shape
...
# ==================================================================
# 4. الواجهة الرئيسية (V10 مع دمج خوارزميات OCR)
# ==================================================================
...
class MedicalDocAppV10(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.btn_smart_crop = self._mk_btn("✂️ قص ذكي (OCR)", "#7c3aed", h=32)
    ...
    def _connect_signals(self):
        ...
        self.btn_smart_crop.clicked.connect(self._do_smart_crop)   # ← نقطة الدمج الجديدة
    ...
    # ==================================================================
    # عمليات التكبير والتدوير والقص الذكي (المطورة)
    # ==================================================================
    ...
    # ------------------------------------------------------------------
    # دالة القص الذكي المتقدمة (OCR) – المحور الأساسي للتطوير
    # ------------------------------------------------------------------
    def _do_smart_crop(self):
        """تنفيذ القص الذكي المتقدم (OCR) باستخدام smart_ocr_get_crop"""
        if self.current_img is None:
            return
        self._push_undo()
        try:
            # استدعاء خوارزمية OCR المتقدمة (القص التكيفي + تصحيح الميلان)
            (l, t, r, b), detected_angle = smart_ocr_get_crop(self.current_img, padding=40, deskew=True)
            # إذا تم تصحيح الميلان، نحدث معلمة deskew_angle
            ...
            self.lbl_crop_val.setText(f"{l}px")   # نعرض الهامش الأيسر فقط للتوحيد
            # مزامنة SpinBoxes
            for sp, val in [(self.sp_crop_l, l), (self.sp_crop_t, t),
                            (self.sp_crop_r, r), (self.sp_crop_b, b)]:
                sp.blockSignals(True)
                sp.setValue(val)
                sp.blockSignals(False)
            self._update_preview()
            self._log(f"✂️ قص ذكي (OCR): L={l} T={t} R={r} B={b}")
        except Exception as e:
            self._log(f"⚠️ خطأ في القص الذكي: {e}")
            QMessageBox.warning(self, "تنبيه", f"فشل القص الذكي: {e}")

    # ==================================================================
    # حفظ وتخطي ومعالجة دفعة
    # ==================================================================
