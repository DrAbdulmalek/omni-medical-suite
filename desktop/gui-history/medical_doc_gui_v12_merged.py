...
def extract_page_number(img: np.ndarray, region: tuple = None) -> int:
    """Extract page number from corners OR user-defined region using OCR."""
    if not OCR_SUPPORT:
        return 0
    ...
    if region:
        # region = (x_frac, y_frac, w_frac, h_frac) as fractions
        x = int(region[0] * w)
        y = int(region[1] * h)
        rw = int(region[2] * w)
        rh = int(region[3] * h)
        regions = [gray[y:y+rh, x:x+rw]]
    else:
        regions = [
            gray[h - 150:h, w - 150:w],  # أسفل يمين
            gray[h - 150:h, 0:150],      # أسفل يسار
            gray[0:150, w - 150:w],      # أعلى يمين
            gray[0:150, 0:150],          # أعلى يسار
        ]
...
# ════════════════════════════════════════════════════════════════
#  Embedded Region Selector (from region_selector.py)
# ════════════════════════════════════════════════════════════════
...
class MedicalDocApp(QMainWindow):
        ...
        try:
            ...
            if img is None:
                ...
                if self.chk_smart_save.isChecked() and OCR_SUPPORT:
                    region = getattr(self, '_page_number_region', None)
                    page_num = extract_page_number(processed, region)
                    if page_num > 0:
    ...
    def _select_page_number_region(self):
        ...
        pix = cv2_to_pixmap(self.current_img, max_w=800, max_h=600)
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            if region_px:
                h, w = self.current_img.shape[:2]
                # تحويل إلى كسور (0-1) لتكون مستقلة عن حجم الصورة
                self._page_number_region = (
                    region_px.x() / w,
                    region_px.y() / h,
                    region_px.width() / w,
                    region_px.height() / h
                )
                self._log("📍 منطقة رقم الصفحة محددة: {}".format(self._page_number_region))
                # اختبار فوري
                self._test_page_number_region()

    def _test_page_number_region(self):
        """اختبار OCR على المنطقة المحددة."""
        if not hasattr(self, '_page_number_region') or not self._page_number_region:
            return
        num = extract_page_number(self.current_img, self._page_number_region)
        if num > 0:
            QMessageBox.information(self, "✅ نجاح", "تم اكتشاف رقم الصفحة: {}".format(num))
            self._log("🧪 OCR تجريبي: صفحة {}".format(num))
        else:
            QMessageBox.warning(self, "⚠️ فشل", "لم يتم اكتشاف رقم في المنطقة. حاول تعديلها.")
    ...
    def analyze_and_organize_pages(self):
        ...
        for idx, entry in enumerate(self.image_list):
            try:
                ...
                region = getattr(self, '_page_number_region', None)
                page_num = extract_page_number(img, region)
